...
const G = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  html, body { background: ${C.bg}; color: ${C.text}; font-family: ${C.sans}; font-size: 16px; line-height: 1.7; -webkit-font-smoothing: antialiased; overflow-x: hidden; }
  p, li, span { font-style: normal !important; }
  textarea { resize: vertical; font-family: ${C.sans}; font-size: 15px; line-height: 1.75; }
  ...
  @media print { nav { display: none !important; } button { display: none !important; } body { background: white !important; color: black !important; } }
  @media(max-width:600px) { .score-card-grid { grid-template-columns: 1fr !important; } }
`;
        ...
        <div style={{ minHeight: "100vh", background: C.bg, paddingTop: 64, overflowX: "hidden", opacity: resultVisible ? 1 : 0, transition: "opacity 0.3s ease" }}>
          ...
          {menuOpen && (
            <div className="mobile-menu open">
              <button onClick={() => { setResult(null); setScreen("input"); setActiveTab("fit"); setMenuOpen(false); }} style={{ padding: "12px 16px", borderRadius: 10, fontFamily: C.sans, fontSize: 14, fontWeight: 500, cursor: "pointer", border: `1px solid ${C.accent}`, background: C.accent, color: "#fff", display: "flex", alignItems: "center", gap: 10 }}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 2v3M8 11v3M2 8h3M11 8h3" stroke="#fff" strokeWidth="1.3" strokeLinecap="round"/><circle cx="8" cy="8" r="3" stroke="#fff" strokeWidth="1.3"/></svg>Analyse New Role
              </button>
              <button onClick={() => { const txt = buildAnalysisText(result); navigator.clipboard.writeText(txt); setCopiedAll(true); setTimeout(() => setCopiedAll(false), 2000); setMenuOpen(false); }} style={{ padding: "12px 16px", borderRadius: 10, fontFamily: C.sans, fontSize: 14, fontWeight: 500, cursor: "pointer", border: "1px solid rgba(16,185,129,0.25)", background: "rgba(16,185,129,0.08)", color: C.green, display: "flex", alignItems: "center", gap: 10 }}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="5" y="1" width="9" height="11" rx="2" stroke="#10B981" strokeWidth="1.3"/><rect x="2" y="4" width="9" height="11" rx="2" stroke="#10B981" strokeWidth="1.3"/></svg>{copiedAll ? "Copied ✓" : "Copy Analysis"}
              </button>
              <button onClick={() => { window.print(); setMenuOpen(false); }} style={{ padding: "12px 16px", borderRadius: 10, fontFamily: C.sans, fontSize: 14, fontWeight: 500, cursor: "pointer", border: `1px solid ${C.border}`, background: "transparent", color: C.textMuted, display: "flex", alignItems: "center", gap: 10 }}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 2v8M5 7l3 3 3-3" stroke="#8892B0" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/><path d="M2 12h12" stroke="#8892B0" strokeWidth="1.3" strokeLinecap="round"/></svg>Save as PDF
              </button>
            </div>
          )}
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "clamp(16px,3vw,28px) clamp(16px,4vw,28px) 40px", textAlign: "left" }}>
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                    style={{ flex: 1, minWidth: 0, padding: isActive ? "13px 16px" : "10px 16px", border: `1px solid ${isActive ? "#4A3EC0" : "#1A1C30"}`, borderBottom: isActive ? `1px solid ${C.surface}` : `1px solid #4A3EC0`, borderRadius: "10px 10px 0 0", background: isActive ? C.surface : "#0A0B14", color: isActive ? C.text : "#3D4060", fontFamily: C.sans, fontSize: 13, fontWeight: isActive ? 500 : 400, cursor: "pointer", transition: "all 0.2s", transform: isActive ? "translateY(0)" : "translateY(6px)", marginBottom: isActive ? "-1px" : "0", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, zIndex: isActive ? 10 : 1, position: "relative", whiteSpace: "nowrap", overflow: "hidden", margin: "0 -1px" }}>
            ...
            {activeTab === "fit" && (
              <div style={{ animation: "fadeUp 0.3s ease both", background: C.surface, border: "1px solid #4A3EC0", borderRadius: "0 12px 12px 12px", padding: "clamp(16px,3vw,24px)", marginBottom: 16, position: "relative", zIndex: 0 }}>
                ...
                <div className="grid-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12, textAlign: "left" }}>
                  <Card>
                    <SectionLabel color={C.amber}>Hiring Manager Lens</SectionLabel><BulletList items={hm} color={C.amber} /><Divider />
                    <SectionLabel color={C.red}>Red Flags They'll Notice</SectionLabel><BulletList items={flags} color={C.red} />
                  </Card>
                  <Card>
